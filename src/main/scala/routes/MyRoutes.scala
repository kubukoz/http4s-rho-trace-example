...
class MyRoutes[F[_]: Async: Trace: Logger](swaggerSyntax: SwaggerSyntax[F], redisRepository: RedisRepository[F]) extends RhoRoutes[F] {
  ...
  //I couldn't get this to compile quick without the covariance (rho's types are... tough. honestly the reason why I stay away)
  //so you're gonna have to figure this one yourself 😅
